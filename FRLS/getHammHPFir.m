function [h] = getHammHPFir(M,fCut,fSamp)
    ...
    % inputs
    % M = order of the high-pass filter
    % fCut = high-pass frequency in Hz
    ...
    for i = 0:(floor((M-1)/2)-1) %we can do with half since it is symmetric
        ...
        h(i+1,1) = ((sin(pi*(n-tau)))/(pi*(n-tau)) - (sin(pi/K*(n-tau)))/(pi*(n-tau)))*hammWin;
        n = n+1;
    end
