...
% dynamic Wiener filter compared to the NLMS algorithm
...
% addpath to several subroutines when running within the Virgo
...
outReadStruct.savePathNLMS = [currDir,'/Output/NLMS/'];
...
% prepare the NLMS input structure
nlms = setNLMSStruct(nChannels,firOrder);
...
% to check that the input data covariance matrix is not full rank
...
for winNo = 1:1:outReadStruct.nWin
    ...
    outDataStruct.saveNameNLMS = [outReadStruct.savePathNLMS,startTimeStr,'.mat'];
    ...
    % scaling by precomputed scaling factor
    ...
    [nlms,dEstNLMS,errNLMS,filtOut] = doNLMS(outDataStruct,nlms);
    ...
    for mm = 1:1:100
        ...
        [nlmsFFTErr,~] = pwelch(dEstNLMS(sm1:sm2,1)-tarData(sm1:sm2,1),100,50,100,100);
        ...
        errAll10_15(mm,2) = 10*log10(mean(nlmsFFTErr(f1Ind:f2Ind,1))/mean(tFFT(f1Ind:f2Ind,1)));
        
        errAll15_20(mm,1) = 10*log10(mean(wFFTErr(f2Ind:f3Ind,1))/mean(tFFT(f2Ind:f3Ind,1)));
        errAll15_20(mm,2) = 10*log10(mean(nlmsFFTErr(f2Ind:f3Ind,1))/mean(tFFT(f2Ind:f3Ind,1)));
        
        errAll20_25(mm,1) = 10*log10(mean(wFFTErr(f3Ind:f4Ind,1))/mean(tFFT(f3Ind:f4Ind,1)));
        errAll20_25(mm,2) = 10*log10(mean(nlmsFFTErr(f3Ind:f4Ind,1))/mean(tFFT(f3Ind:f4Ind,1)));
